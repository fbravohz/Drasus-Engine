...
mod tests {
    ...
    async fn record_event_exhausts_exactly_max_attempts_when_write_lock_is_held() {
        ...
        let temp_dir = tempfile::tempdir().expect("crear directorio temporal");
        let db_path = temp_dir.path().join("domain_events_forced_contention.sqlite");
        let database_url = format!("sqlite://{}", db_path.display());
        ...
        // Escritor B: intenta registrar un evento mientras A retiene el
        // lock. Cada `try_record_event_once` abre `BEGIN IMMEDIATE`, choca
        ...
        match result {
            Err(DomainEventRepositoryError::WriteContention { attempts }) => {
                assert_eq!(
    ...
    async fn is_transient_is_false_for_a_permanent_non_sequence_unique_violation() {
        ...
        let permanent = DomainEventRepositoryError::Database(err);
        assert!(
        ...
        let non_database = DomainEventRepositoryError::WriteContention { attempts: 5 };
        assert!(
    ...
    /// `record_event` es bit-a-bit idéntica a la fila persistida en disco --
    /// si el literal de retorno de `try_record_event_once` sustituyera
