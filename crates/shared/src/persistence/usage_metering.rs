...
mod tests {
    ...
    /// `MAX_RECORD_OPERATION_ATTEMPTS` intentos y rendirse con
    /// `WriteContention { attempts: MAX }` -- nunca descartar la operación en
    ...
    /// "database is locked" como transitorio. Patrón de referencia:
    /// `persistence/data_portability.rs` (STORY-043, DEBT-018).
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn record_operation_exhausts_exactly_max_attempts_when_write_lock_is_held() {
        use std::str::FromStr;
        ...
        let temp_dir = tempfile::tempdir().expect("crear directorio temporal");
        let db_path = temp_dir.path().join("usage_metering_forced_contention.sqlite");
        let database_url = format!("sqlite://{}", db_path.display());
        ...
        // Escritor B: intenta registrar una operación mientras A retiene el
        // lock. Cada `try_record_operation_once` abre `BEGIN IMMEDIATE`,
        // choca con el lock de A, falla con "database is locked"
        // (transitorio) y reintenta, hasta agotar MAX_RECORD_OPERATION_ATTEMPTS.
        ...
        match result {
            Err(UsageRepositoryError::WriteContention { attempts }) => {
                assert_eq!(
                    attempts, MAX_RECORD_OPERATION_ATTEMPTS,
                    "bajo contención sostenida debe agotar EXACTAMENTE MAX_RECORD_OPERATION_ATTEMPTS intentos"
                );
            }
            other => panic!(
                "se esperaba WriteContention {{ attempts: {MAX_RECORD_OPERATION_ATTEMPTS} }} bajo contención sostenida, se obtuvo: {other:?}"
            ),
        }
    }
    ...
    async fn is_transient_is_false_for_a_permanent_non_sequence_unique_violation() {
        ...
        let permanent = UsageRepositoryError::Database(err);
        assert!(
        ...
        let non_database = UsageRepositoryError::UnknownQuotaVerdict("X".to_string());
        assert!(
    ...
    /// `record_operation` es bit-a-bit idéntica a la fila persistida en
    /// disco -- si el literal de retorno de `try_record_operation_once`
    /// sustituyera algún campo (`audit_hash`, `event_sequence_id`,
    /// timestamps...) por un valor por defecto en vez del recién calculado,
    /// esta comparación de igualdad completa lo detectaría.
    #[tokio::test]
