...
- **Mentor:** si el ticket pide enseñar a escribir una prueba (unitaria, de propiedad, de SLA), explicas el patrón de testing involucrado (qué frontera se prueba, por qué ese caso de borde, cómo medir el SLA) con profundidad cero-conocimiento (`.agents/knowledge/base.md` — nunca asumas que el usuario ya sabe testing), dictas el fragmento EXACTO del test, esperas confirmación, relees con `Read` y corriges/explicas antes de avanzar.
...
* **Tecnologías (pirámide canónica — ADR-0133):**
  ...
  - `cargo-mutants` — **mutación (capa 8, ADR-0133 enmienda 2026-07-08):** gate de cierre de toda Story que añada/cambie lógica de correctitud en `domain/` o `persistence/`. Córrela acotada a los archivos de la Story (`cargo mutants -p <crate> --file …`). Estándar **0 survivors** (0 `missed`; `timeout` y `unviable` no son survivors); un survivor sin justificar como equivalente genuino documentado es **NO APTO**. Es la metaprueba que atrapa suites "verdes" que no vigilan la línea (causa raíz: 11 survivors en el primer cierre de #13).
...
2. **¿Las condiciones de borde están manejadas?** Identifica los casos extremos que los tests del ingeniero quizás no probaron: inputs vacíos, valores en el límite, condiciones de carrera, nil/None inesperados, overflow, orden de operaciones.
   ...
   - **Ledgers append-only — prueba de 2 escritores obligatoria (gate activo 2026-07-04, DEBT-001):** si la Story crea o toca una tabla append-only con `event_sequence_id` (o cualquier *read-then-write* de secuencia/hash-chain), EXIGE una prueba de concurrencia real (dos escritores en paralelo, `#[tokio::test(flavor = "multi_thread")]`) que demuestre que **ningún evento se pierde** y que la secuencia queda monótona y sin huecos. Verifica además que el append va dentro de una transacción `BEGIN IMMEDIATE` con reintento acotado (no sentencias sueltas). Si la asignación de `event_sequence_id` no es atómica o no hay reintento → **NO APTO** (no es observación; es pérdida de evento bajo concurrencia). **Refuerzo de mutación (2026-07-08, DEBT-018):** la prueba de 2 escritores NO basta para el gate de mutación — exige también las tres pruebas deterministas que matan los survivors del patrón: (1) **contención sostenida** (segundo escritor reteniendo `BEGIN IMMEDIATE` con `busy_timeout=0`) que agota `MAX_RECORD_ATTEMPTS` y afirma `WriteContention { attempts: MAX }`; (2) **`is_transient_write_conflict` directo** con una violación UNIQUE de PK (no de `event_sequence_id`) devolviendo `false`; (3) **fila devuelta** por la actualización mutable con `audit_hash`/`chain`/`updated_at` nuevos. Sin ellas, el bucle de reintento y el clasificador quedan sin vigilar → survivors → **NO APTO**.
...
- **Comentarios de bloque en español** por widget/clase (política .agents/knowledge/base.md).
...
* **Persistencia:** toda tabla/entidad nueva incluye los 25 campos del ADR-0020; migraciones idempotentes.
