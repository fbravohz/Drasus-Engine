...
* **Qué verificas tú (mínimo, según la tarea):**
  ...
  * **Modelo de Datos (`docs/DATA-MODEL.md`):** si la Story tocó `migrations/*.sql`, confirmas que `docs/data-model/<tabla>.md` quedó creada/actualizada en el mismo cambio (y la fila del índice `docs/DATA-MODEL.md` si es tabla nueva) — comparas columna por columna contra la migración real, no contra el resumen del ingeniero. Sin esto, no cierras la Story aunque el resto esté verde.
...
0. **Gate de Coherencia Pre-Despacho (OBLIGATORIO antes de crear la Orden):** antes de redactar cualquier Orden de Trabajo, auditas la Feature spec y sus TTRs contra las decisiones de arquitectura vigentes. Checklist:
   ...
   - **Sincronización del Modelo de Datos (`docs/DATA-MODEL.md` — OBLIGATORIO desde 2026-07-12, ADR-0153):** toda Story/Task que crea o modifica una migración en `migrations/*.sql` actualiza, en el MISMO cambio, la ficha correspondiente en `docs/data-model/<tabla>.md` (plantilla: `docs/templates/DATA-MODEL.md`) — tabla nueva → ficha nueva + fila nueva en el índice `docs/DATA-MODEL.md`; columna/FK/CHECK modificado → ficha actualizada. A diferencia de ADR/Feature/TTR, esta ficha SÍ refleja el detalle SQL literal (columna, tipo, FK) — es la excepción documentada a "Lo Prohibido" de `docs/templates/TEMPLATES.md`. Si tocaste la migración y no la ficha, la Story no está completa — no la cierres.
...
6. **`docs/DATA-MODEL.md`**: índice del modelo de datos real (una fila por tabla SQL existente, con su feature dueña y FKs). Consúltalo antes de tocar cualquier migración — confirma si la tabla ya existe/tiene ficha (`docs/data-model/<tabla>.md`) antes de crear una nueva, y para entender relaciones entre tablas sin releer `migrations/*.sql`.
