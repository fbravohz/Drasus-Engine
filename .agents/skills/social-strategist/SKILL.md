...
Usa la herramienta Read para leer el archivo completo `.agents/knowledge/base.md`. Ese archivo contiene las reglas de rigor operativo que gobiernan este skill y tiene supremacía absoluta.
Declara: `[.agents/knowledge/base.md leído y activo]` antes de continuar.
...
1. **Lee `.agents/state/social-strategist/PROGRESS.md`.** Si no existe, créalo vacío con la plantilla de la sección "Gestión de Archivos" — es tu memoria entre sesiones.
...
**Fuente primaria de contexto:** `docs/execution/` para el detalle de cada Story/Task + `git log` para commits. Consulta `.agents/state/tech-lead/PROGRESS.md` solo si hay dudas sobre el estado o decisiones arquitectónicas — es una bitácora densa; no la leas por defecto. El usuario también puede indicar cuándo revisarla.
...
4. Guarda en `.agents/documents/social-strategist/social-strategy-[YYYY-MM-DD]-es.md` y `-en.md` (plantilla abajo).
5. Actualiza `.agents/state/social-strategist/PROGRESS.md`: marca el cierre como procesado y registra el nuevo `último_commit_procesado`.
...
6. Guarda todo en `.agents/documents/social-strategist/episodios/[slug]/`: `guion.md`, `assets/`, `checklist.md`.
7. Aplica idioma según `CONTENT-STRATEGY.md` §1.5.
8. Actualiza `.agents/state/social-strategist/PROGRESS.md`: marca qué Historias/Riesgos/decisiones documentadas quedaron "cubiertos" por este episodio (regla de no duplicación, §10).
...
4. Guarda igual que el Pipeline A, etiquetando la entrada como "Pilar F / postura" en `.agents/state/social-strategist/PROGRESS.md`.
...
6. **Guarda** en `.agents/documents/social-strategist/capsulas/[concept-slug]-[YYYY-MM-DD]-es.md` y `-en.md` (plantilla abajo).
...
## `.agents/state/social-strategist/PROGRESS.md` (memoria del skill — créalo si no existe)
...
> Ignora `.gitkeep` en `.agents/documents/social-strategist/` — solo mantiene el directorio en git.
