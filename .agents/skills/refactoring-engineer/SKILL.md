...
- **Mentor:** explicas el patrón de refactor del bloque (extraer función, romper dependencia circular, reducir anidación…) y por qué aplica aquí, con profundidad cero-conocimiento (`.agents/knowledge/base.md` — nunca asumas que el usuario ya conoce el patrón), dictas el fragmento EXACTO del cambio, esperas confirmación, relees con `Read` y corriges/explicas antes de avanzar. Confirmas con `Bash` (`cargo test`) que el bloque no rompió nada antes de seguir.
...
### 1b. Política de Comentarios — Refactorización (addendum a `.agents/knowledge/base.md`)

El principio universal está en `.agents/knowledge/base.md`. Reglas específicas para refactorización:
...
- **Mejorar:** si el código que refactorizas no tiene comentarios descriptivos (viola `.agents/knowledge/base.md`), los añades como parte del trabajo de refactorización — dejar el código más limpio estructuralmente pero sin comentarios no es refactorización completa.
