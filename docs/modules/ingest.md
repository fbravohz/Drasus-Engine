...
Toda entidad persistida (Barras, Regímenes) incluye el set de relevancia técnica para Datos:

| Categoría | Campo | Descripción |
| :--- | :--- | :--- |
| **I. Identidad** | `id` | Identificador único del item de datos |
| | `created_at` | Timestamp de ingesta/proceso |
| | `audit_hash` | Hash de integridad del bloque/archivo |
| | `audit_chain_hash` | Hash de la secuencia cronológica |
| **II. Linaje** | `source_id` | Ref a la fuente original (Broker/S3) |
| | `transformation_id` | ID del paso de transformación/limpieza |
| | `logic_hash` | Hash del driver o motor de proceso |
| **III. Hardware** | `node_id` | ID del hardware físico de ingesta |
| | `process_id` | PID del worker de datos |
