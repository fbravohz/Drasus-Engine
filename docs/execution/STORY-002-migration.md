...
- **ADR(s):** ADR-0006 (migraciones centralizadas SQLx), ADR-0020 (los 25 campos maestros)
...
(dónde viven las migraciones y cómo). 4) Lee ADR-0020 (LISTA EXACTA de los 25 campos — NO inventes
ni uno). 5) Lee ROADMAP §EPIC-0 Sprint 0 fila STORY-002.

ORDEN: Crear la migración 0001 con SQLx, que cree los 25 campos maestros de ADR-0020 en SQLite WAL.
- Migraciones embebidas en la ubicación que dicte ADR-0006 (centralizadas).
- 25 campos EXACTOS de ADR-0020 (nombres, tipos, semántica).
...
tipos. Si ADR-0020 es ambiguo en un campo, repórtalo como BLOQUEO con cita. Si ADR-0006 y ADR-0020
...
- La migración aplica los 25 campos exactos de ADR-0020 en SQLite WAL.
...
- 2026-06-12 · Rust-Engineer (Sonnet) · **APROBADO** · Auditoría Tech-Lead: 25 campos verificados uno a uno contra ADR-0020 (líneas 395-399); `migrations/0001_foundation_master_fields.sql`; test `migration_0001_applies_in_wal_mode_and_is_idempotent` verde; WAL e idempotencia confirmadas.
- 2026-06-12 · **Escalamiento a Architect (Opus)** · El ingeniero marcó dudas de diseño (forma del contrato; `transformation_id`). Veredicto: los 25 campos son **contrato lógico + filtro por perfil**, NO molde físico de 25 columnas en cada tabla; la tabla ancla de EPIC-0 es correcta. `transformation_id` es identificador, no flag. Propagado a ADR-0020, SAD §17.9/§20 y 8 módulos (aprobado por el usuario).

## 7. Pendientes derivados / decisiones
- **Implicación para Sprint 1:** las tablas de las Stories STORY-003–STORY-008 NO copian 25 columnas; aplican el Filtro de Relevancia por Perfil (ADR-0020 actualizado).
