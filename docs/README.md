...
| [**asset-exposure-map**](./features/asset-exposure-map.md) | Resuelve de forma determinista qué evento fundamental aplica a qué activo y con qué peso, mediante un vector de exposición (emisor, sector, país, divisa...) — base del indicador relativo por instrumento (ADR-0128). | generate, manage |
...
| [**event-impact-scorer**](./features/event-impact-scorer.md) | Motor determinista que convierte un evento fundamental en un coeficiente de impacto por fórmula (Event Study sobre la reacción de precio, o Surprise sobre el consenso previo), nunca por opinión (ADR-0125). | generate |
...
| [**fundamental-event-store**](./features/fundamental-event-store.md) | Almacén PIT de eventos fundamentales (macro, resultados, política monetaria): ingesta el hecho crudo con linaje y lo versiona vintage/as-of (first-print + revisiones) sin look-ahead (ADR-0126/0127). | ingest |
| [**fundamental-indicator-projector**](./features/fundamental-indicator-projector.md) | Proyecta el evento puntuado a una serie de indicador estándar, normalizada por instrumento, consumible por las estrategias igual que un indicador técnico — sin lógica fundamental en el hot-path (ADR-0128). | generate, validate, execute, manage |
