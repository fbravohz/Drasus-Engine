...
**Criterio de salida:** migración greenfield con las tablas de los 9 cimientos **+ el registro #10 + #11 + #12 + #13 + #14** (Grupo I + perfil, `STRICT`, UUIDv7); el bus emite los eventos enriquecidos **incluidos flujo de capital y snapshot de estado de cuenta (ADR-0145)**; el gate de licencia suprime/reactiva la telemetría según tier con período de gracia offline; el firehose gratuito captura artefactos de trabajo (sin secretos); el consentimiento se registra versionado **con la distinción de dos categorías (gate de tier vs. opt-in genuino, ADR-0148/consent-registry)**; el evaluador de permisos de `agentic-mcp-gateway` queda extendido con el rol del operador (ADR-0149). Cada cimiento con Superficie propia entrega su SVF; los de plomería registran su Ventana de Verificación (ADR-0117).
