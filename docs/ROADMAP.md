...
- **Vocabulario:** los identificadores internos (`EPIC-n`, `STORY-###`, `TTR`, `ADR-XXXX`) están traducidos en `.agents/knowledge/base.md` ("Habla en Cristiano"). Aquí: **EPIC = fase/gran bloque**, **STORY = trabajo con código**, **SPIKE = investigación de un riesgo técnico**, **TASK = trabajo sin código**.
...
| **Cimientos** | **Substrato de Monetización** (transversal, greenfield) | `shared` + Cabina de Mando | Tres Planos (ADR-0143) + 13 cimientos (ADR-0144/0145/0146/0147/0148) listos para cualquier modelo de negocio | pendiente (va **antes** de la auditoría retroactiva) |
...
| **Cabina de Mando** | Servidor central del proveedor + cara web pública — repo hermano [`drasus-portal`](../../drasus-portal/README.md) (Plano #3, ADR-0143; fuera del monolito, stack libre ADR-0145) | — (repo aparte) | Auth · licencias · facturación/pagos · ingesta+agregación de telemetría · panel público de Cuentas Verificadas · marketplace | pendiente — **sin nº de EPIC por secuenciación**; disparador: **primer cobro real (tras EPIC-5)**; precondición: Spike gRPC (ADR-0142). Detalle en el bloque "Diferido — anclaje y disparadores" de Cimientos de Monetización. |
...
**Objetivo:** esqueleto del workspace hexagonal (ADR-0137) compilando, con las fundaciones anti-retrabajo (ADR-0020) y los 6 gates de viabilidad resueltos.

**Alcance:** workspace Cargo hexagonal (ADR-0137): `shared` (catálogo de tipos + plomería de infraestructura), `crates/features/<dominio>/` (un crate hexagonal por feature de dominio — vacío al cierre de EPIC-0, se puebla desde EPIC-1), `presets/` (cableado sin lógica), `app` (binario) y `bridge` (FFI). Las 6 features de Fundación viven en `shared` como infraestructura crosscutting bendecida (ADR-0137 enmienda 2026-06-23), no como crates propios. Migración 0001 con la tabla ancla `foundation_master_fields` (catálogo de 25 campos, ADR-0020); features transversales de plomería [`clock`](./features/clock.md), [`audit-log`](./features/audit-log.md), [`telemetry`](./features/telemetry.md), [`async-job-executor`](./features/async-job-executor.md) (ADR-0011), [`worker-isolation-orchestrator`](./features/worker-isolation-orchestrator.md), [`agentic-mcp-gateway`](./features/agentic-mcp-gateway.md) (núcleo del servidor MCP + evaluador de permisos, ADR-0123); CLI con Clap + binario raíz `app` (SAD §4.2); Panel Operativo Fundacional (SPIKE-006/ADR-0117), cuyo Banco de Verificación se formaliza como [`verification-bench`](./features/verification-bench.md) y gate permanente de cierre (ADR-0152).
...
| **STORY-025 — Librería de Componentes Funcional, Neutral al Estilo y al Proyecto (ADR-0138 enmienda 2026-06-29)** — 51 componentes en `ui/lib/components/` (clases desnudas namespaced, funcionales, estilo por tema), galería→consumidora con mocks, tema `Drasus*`→neutral, índice `ui/COMPONENTS.md` | ✅ terminado (2026-07-03, build verde; alias `custom_ui`; `kAppName`; SVF STORY-024 reescrita; dedup `ColorPickerWidget` resuelto) | [Orden](./execution/STORY-025-component-library.md) |
| **STORY-026 — Cimientos de UI a la capa de tema** — mover tokens `Gx` (→`theme/gx_tokens.dart`) y helpers de superficie (`frosted`/`panelSurface`/`cardSurface`/`glassEnhanced`/`PanelFromDecoration` →`theme/surfaces.dart`) de `gallery/` a `theme/`; elimina la dependencia de capas invertida `components/→gallery/` (80 archivos repunteados) | ✅ terminado (2026-07-03, build verde; `theme/` sin imports a `gallery/`) | [Orden](./execution/STORY-026-theme-foundation-move.md) |
...
**Alcance:** los **9 cimientos** (ADR-0144) **+ el cimiento #10 `verified-account-registry`** (ADR-0145) **+ #11 `instance-continuity`** (ADR-0146) **+ #12 `master-account-hierarchy`** (ADR-0147) **+ #13 `data-portability`** (ADR-0148), plomería crosscutting que vive en `crates/shared` (patrón `clock`/`audit-log`/`telemetry`) + la nueva superficie de la Cabina de Mando. Cada uno entrega su puerto tipado (ADR-0137) y su esquema greenfield (ADR-0141/ADR-0020) **ahora**; los productos concretos (y el portal público, repo aparte) son adaptadores posteriores.
...
| 10 | [`verified-account-registry`](./features/verified-account-registry.md) (ADR-0145) | Cuentas Verificadas: atestación soberana + read-only al bróker + registro multi-cuenta + track record publicable |
| 11 | [`instance-continuity`](./features/instance-continuity.md) (ADR-0146) | Respaldo cifrado + maestro itinerante (relevo de custodia entre máquinas activadas) |
| 12 | [`master-account-hierarchy`](./features/master-account-hierarchy.md) (ADR-0147) | Cuenta maestra raíz (fondo) con auditoría/override sobre cuentas maestras hijas autónomas |
| 13 | [`data-portability`](./features/data-portability.md) (ADR-0148) | Catálogo de tablas con `owner_id` + registro de solicitudes de exportación/olvido (GDPR Art. 15/17/20) |
| 14 | [`operator-roles`](./features/operator-roles.md) (ADR-0149) | Roles de operador a la carta dentro de una cuenta maestra, gateados por puerto de Feature; asignables a humanos o agentes LLM |

**Pilar de Cuentas Verificadas (ADR-0145):** análogo a myFXbook / MT5 Signals con diferenciador soberano (Drasus atestigua por hash chain lo que su motor ejecutó). Enriquece el cimiento #6 con dos eventos nuevos (**flujo de capital** depósito/retiro/transferencia + **snapshot de estado de cuenta** equity/balance/margen) y refuerza la orden-con-fricción con `account_id`/PnL/MAE/MFE/duración — necesarios para reconstruir gain%, curvas y drawdown. Atestación fase 1 = **soberana + read-only al bróker** (la read-only corre en el Plano de Ejecución del usuario; la investor password nunca sube, ADR-0093). El **track record publicable** es telemetría clase 5 (opt-in por cuenta, **independiente del tier**). El **portal público es un repo aparte** (stack libre, p. ej. Next.js; NO el monolito Rust+Flutter — ADR-0001 acotado al monolito) y está **diferido**: ahora solo se cimentan, dentro del monolito, los eventos de #6, el registro #10, el consentimiento #5 y el contrato/puerto de reporte. Ruta pública (`/audited-accounts` vs `/official-accounts`) y stack del portal = puntos abiertos, no bloquean.

**Cimientos #11/#12 (ADR-0146/ADR-0147, 2026-07-06):** `instance-continuity` promueve el moonshot `encrypted-local-backup` a cimiento y añade el **maestro itinerante** (relevo de custodia: una sola máquina titular de la cadena de auditoría en cada instante; cesión/reclamo al cerrar/abrir sesión; liberación forzada desde el panel central si la máquina anterior murió). `master-account-hierarchy` modela una **cuenta maestra raíz** (fondo) con autoridad de auditoría/override sobre **cuentas maestras hijas** autónomas (cada una con su propio Plano de Control ADR-0119); el mando viaja por el **relé cifrado genérico** de ADR-0143 (mismo canal que ahora también reusa `copy-trading-engine`, ADR-0092 enmendado) y todo override queda **atestado en ambos extremos** para no romper la integridad de #10.

**Cimiento #13 (ADR-0148, 2026-07-07):** `data-portability` — catálogo declarativo de tablas con `owner_id` (auto-poblado por cada feature nueva, mismo mecanismo de propagación que ADR-0020) + registro append-only de solicitudes de exportación/olvido. Obligación legal (GDPR Art. 15/17/20), no producto: barato de cimentar ahora porque `owner_id` ya es universal, carísimo de reinstrumentar bajo plazo legal de 30 días con usuarios reales. Distinto de `instance-continuity` (#11, blob opaco de disaster recovery) y de `data-aggregation` (#9, agregado anónimo saliente a terceros). El olvido pseudonimiza en vez de purgar físicamente los registros con retención legal obligatoria (auditoría financiera, atestaciones de #10). Producto de la auditoría de cobertura del substrato — barrido de las ~154 features y ~54 moonshots buscando cimientos faltantes.
...
**Diferido a fase posterior — anclaje y disparadores (para que "diferido" no sea "olvidado"):** el substrato deja **puertos y esquemas locales listos ahora**; lo que se aplaza queda anclado aquí con su disparador, y su deuda granular se enrola en [`DEBT.md`](./DEBT.md) cuando el Tech-Lead llega a ella (regla del propio DEBT: *"si una deuda no está aquí, no está rastreada"*).

| Pieza diferida | Disparador (cuándo se paga) | Registro |
|---|---|---|
| **Cabina de Mando Central** — repo hermano [`drasus-portal`](../../drasus-portal/README.md), **aparte del monolito y de stack libre** (NO hereda Rust+Flutter — ADR-0001 acotado por ADR-0145). Alcance: Auth · Billing/pagos · Metering/reconciliación · Device (activaciones + grafo de colisión anti-abuso) · Backup · panel público de Cuentas Verificadas · marketplace. | El **primer cobro real** (tras EPIC-5, "primer dinero real"); antes no hace falta. **Precondición:** Spike gRPC/protos + contrato de reporte (ADR-0142; `verified-account-registry` TTR-004). Formalizar su nº de EPIC es punto abierto de secuenciación → se decide al acercarse EPIC-5. | ROADMAP (mapa de entregas + esta fila) + DEBT al arrancar |
| **Adaptadores de red (sync local→nube)** — detrás de los **mismos** puertos que ya existen (`identity`/`execution_gate`/`usage`/`consent`), hoy stubs locales. | Con la Cabina. Reconciliación = **servidor autoritativo** (fijada en `usage-metering`). | Banners "Pendiente:" de #1–#5 → DEBT |
| **Grafo de colisión anti-abuso** (correlación cross-cuenta; ML de comportamiento a EPIC-9+). | Con la Cabina (grafo) / EPIC-9+ (ML). | Nota en `central-identity` |
| **Panel de Consumo productivo** (nocional vs. límite, tier, upgrade). | **EPIC-8** (Pulido/UI). Su interino de verificación (SVF + galería con mocks) ya está rastreado. | `DEBT-005` + EPIC-8 |
| **Transferencia de licencia** (self-service, cooldown, invalidación remota). | Con la Cabina (invalidación remota). | `licensing-system` |
| **Adaptador de subida/descarga real del respaldo cifrado** contra el almacén de objetos (contrato/esquema de `instance-continuity` #11 ya no está diferido — solo el adaptador de red). | Con la Cabina (almacén de objetos). | [`instance-continuity`](./features/instance-continuity.md) |
| **Adaptador de venta a fondos de inversión** de `master-account-hierarchy` #12 (onboarding comercial). | Demanda de un cliente institucional real. | [`master-account-hierarchy`](./features/master-account-hierarchy.md) |

**Sprint actual — Banco de Pruebas conecta los 14 cimientos (ADR-0152, 2026-07-12):** el substrato cerró 14/14 backends con QA APTO (auditoría retroactiva 2026-07-10), pero ninguno tiene conexión al Banco de Verificación de la UI — solo son verificables por CLI (Canal #2, ADR-0142; `DEBT-005`). ADR-0152 formaliza el harness genérico como [`verification-bench`](./features/verification-bench.md) y lo eleva a **gate permanente de Definición de Terminado**: ninguna feature con superficie propia cierra Etapa 5 (QA) sin su conexión individual ahí. El sprint que abre este ADR conecta retroactivamente las verificaciones de los 14 cimientos (#1–#14) más cualquier otra feature con backend terminado que aún no tenga su conexión individual — paga `DEBT-005`.

**Criterio de salida:** migración greenfield con las tablas de los 9 cimientos **+ el registro #10 + #11 + #12 + #13 + #14** (Grupo I + perfil, `STRICT`, UUIDv7); el bus emite los eventos enriquecidos **incluidos flujo de capital y snapshot de estado de cuenta (ADR-0145)**; el gate de licencia suprime/reactiva la telemetría según tier con período de gracia offline; el firehose gratuito captura artefactos de trabajo (sin secretos); el consentimiento se registra versionado **con la distinción de dos categorías (gate de tier vs. opt-in genuino, ADR-0148/consent-registry)**; el evaluador de permisos de `agentic-mcp-gateway` queda extendido con el rol del operador (ADR-0149). Cada cimiento con Superficie propia entrega su SVF; los de plomería registran su Ventana de Verificación (ADR-0117) — realizadas concretamente conectando cada cimiento al Banco de Pruebas (ADR-0152, ver "Sprint actual" arriba).

**Retrabajo pendiente (2026-07-06, actualizado 2026-07-07):** el Eje B de realidad de capital (`LIVE`/`PAPER`/`DEMO`/`CHALLENGE`) que ADR-0145 exigió para `verified-account-registry` (#10) **ya fue implementado** por STORY-038/DEBT-014 (QA APTO) — pero como columna **nueva** `capital_reality`, **duplicando** `institutional_tag` (Grupo II, ya presente en ambas tablas de la migración `0016` por Perfil D). Auditoría de Inundación de Fundaciones (ADR-0020) detectó la violación de "reutilización antes que creación": dos columnas con el mismo dominio de valores en la misma fila. Corrección ratificada en ADR-0145: **Eje B = `institutional_tag`**, sin columna adicional. Retrabajo real: eliminar `capital_reality` de `verified_accounts`/`attested_track_records`, estampar en `institutional_tag`, ajustar `domain/verified_account_registry.rs` (dominio Tech-Lead → `DEBT.md`).

**Retrabajo pendiente (2026-07-07):** `plan-tier-quota` (#3) ya tiene código (migración `0009` + Core/Shell, STORY-029) sin el límite `MAX_CHILD_ACCOUNTS` que ADR-0149 exige (cuántas cuentas maestras hijas puede crear un fondo, cuota fijada por Drasus). Es un campo más del plan, mismo mecanismo que `NOTIONAL_LIMIT_FREE`/activaciones — no requiere infraestructura nueva, solo añadir el campo antes de que `master-account-hierarchy` (#12) y `operator-roles` (#14) lo necesiten en producción (dominio Tech-Lead → `DEBT.md`).
...
**Alcance:** la mitad "núcleo" de `validate` (ver su tabla de TTRs, fase EPIC-2 en [`docs/modules/validate.md`](./modules/validate.md#ttrs-etiquetados-por-fase)): motor de backtest dual (ADR-0114), fricción institucional (`slippage-models`, Bar-Open Alignment, ADR-0017), métricas duales (ADR-0047), `equity-curve-tracker`, `precision-sizing-models` (ADR-0044), `executable-container` (ADR-0009), `strategy-versioning` (ADR-0005), y la **columna vertebral de procedencia** (`pipeline-registry` + `expedition-ledger`, ADR-0150/ADR-0151) — incluye el nivel Workspace de `pipeline-registry` (ADR-0153) — se siembra aquí porque el primer backtest es la primera **Expedition** (corrida) que registrar; la Expedition guarda el **insumo** de deflación (`trials_count` + σ² + sketch de Sharpe + familia a priori) y el pivote de linaje permite reconstruir el N **por punto de decisión** que el DSR/PBO de EPIC-4 aplican (ADR-0151 — el `trials_count` de por vida NO es el N; guardia de límite degenerado). El criterio de salida de EPIC-3 "el N global queda registrado e inmutable" se apoya en esta entidad (registrado como insumo, aplicado por decisión). El compilador AST y el `design-manifest` se construyen aquí porque el motor los necesita, aunque pertenezcan a `generate` (primer consumidor real es el backtest). El motor soporta **N posiciones concurrentes** independientes por señal fresca con de-duplicación por vela (ADR-0129) y es **agnóstico a la temporalidad**: scalping, intradía, swing, posición y ticks son ciudadanos de primera clase (ADR-0130).
...
**Alcance:** el 100% del núcleo de `generate` (ver [`docs/modules/generate.md`](./modules/generate.md#ttrs-etiquetados-por-fase)): `nsga2-optimizer` nativo, AST + WildCards (ADR-0043), `databank-lake`/`databank-manager` (ADR-0055) — incluye su superficie de tabla masiva (Grid View) y el primitivo efímero de Selección (ADR-0153) —, `dsr-tracking-engine` desde la primera corrida (ADR-0067), `parameter-optimization`, `zero-crossing-filter`, detección de régimen [`hmm-regime-detection`](./features/hmm-regime-detection.md) (modelo ajustado offline + etiquetado; EPIC-1 dejó la columna `regime_label='desconocido'` como placeholder válido, SAD §11); capa de indicadores fundamentales: [`event-impact-scorer`](./features/event-impact-scorer.md) (Event Study + Surprise, ADR-0125), [`asset-exposure-map`](./features/asset-exposure-map.md) (relevancia evento→activo por vector de exposición, ADR-0128) y [`fundamental-indicator-projector`](./features/fundamental-indicator-projector.md) (indicador estándar normalizado por instrumento, ADR-0128); frecuencia y horizonte de operación como objetivo declarable en NSGA-II con mínimo de trades configurable y agnosticismo de temporalidad (ADR-0130).
...
> 🔶 **`DEBT-024` — diseño cerrado 2026-07-12:** el modelo de **Pipeline como proceso recurrente** ("corre en bucle hasta que el usuario lo pare/modifique/reinicie") ya está resuelto — `event-driven-pipeline-triggers` TTR-003 (run-state `RUNNING`/`PAUSED`/`STOPPED`, una Expedition nueva por iteración). Queda pendiente su implementación real cuando EPIC-7 se construya. Detalle en [`DEBT-024`](./debt/DEBT-024.md).
...
5. **Versionado (ADR-0005) y los 25 campos (ADR-0020) nacen en EPIC-0/EPIC-2:** retrofitearlos cuesta 10x.
