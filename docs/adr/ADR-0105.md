> 🔶 **Nota de coexistencia (ADR-0030, ADR-0036):** "Exclusivo" aplica al procesamiento de DataFrames en memoria (transformaciones, indicadores, agregaciones). DuckDB mantiene su rol como motor SQL out-of-core sobre Parquet para consultas analíticas masivas y remuestreo dinámico (ADR-0030, ADR-0036). Polars y DuckDB coexisten: Polars para cómputo en memoria, DuckDB para consultas sobre datos que exceden la RAM.
