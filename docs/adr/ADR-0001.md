> 🔶 **Acotado por ADR-0145 (nota de alcance)** — el invariante de stack único **Rust (Core) + Flutter (UI)** gobierna el **monolito** que los usuarios ejecutan (motor + interfaz). NO gobierna la **infraestructura web/servidor del proveedor** (portal público de Cuentas Verificadas, Cabina de Mando web), que vive en un **repositorio aparte** con su propio stack a decidir después (p. ej. Next.js para SEO). La prohibición de React/TS/Python sigue vigente **dentro del monolito**; fuera de él, en el repo del proveedor, no aplica.
...
* **Ver también:** ADR-0003 — Concreción estructural de este ADR (organización de carpetas, reglas de frontera y features reutilizables).
