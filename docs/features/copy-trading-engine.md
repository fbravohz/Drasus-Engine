...
*   `event_sequence_id` (secuencia de recuperación del copiado)
*   `owner_id` (identificador del copier local)
*   `compliance_status_id` (resultado de validación táctica local — Grupo V Gobernanza)
*   `source_signal_id` (señal del máster que originó la copia — Grupo V Latencia)
