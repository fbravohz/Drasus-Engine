...
- **➡️ SIGUIENTE PASO CONCRETO:** retomar cimientos de EPIC-0. Despachar **STORY-006 (`crash-recovery`)** en Sonnet (ojo solape con la recuperación de STORY-005; al leer la feature decidir si extiende o es independiente, escalar al Architect si ambiguo). Luego STORY-007 (`telemetry`), STORY-008 (`worker-isolation`), STORY-009 (CLI + binario raíz `app` + test `kill -9` real diferido). Transversal: los 6 spikes de gates (SPIKE-001–006) antes de cerrar EPIC-0 / arrancar EPIC-1.
