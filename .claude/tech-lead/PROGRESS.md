...
- **➡️ SIGUIENTE PASO CONCRETO (PRIORITARIO, retomar aquí):** Continuar **TASK-004 — Auditoría de la Inundación de Fundaciones** (`docs/execution/TASK-004-auditoria-inundacion-fundaciones.md`). Fase 1 (diagnóstico de 137 features + 8 módulos por 13 agentes Sonnet) COMPLETA; ~36 features ya corregidas (casos claros) y commiteadas. **Falta Fase 2 = SESIÓN DE DECISIONES DE DISEÑO con el usuario:** revisar la tabla A (≈20 perfiles a reasignar) y la tabla B (campos fuera de catálogo + candidatos a campos nuevos: `compliance_status_id`, `portfolio_container_id`, campo de linaje jerárquico). El usuario decidió: PRIMERO decidir diseño, LUEGO corregir todo junto (Fase 3) + arreglar `TEMPLATES.md` (Fase 4). Todo el material de decisión está en el doc TASK-004. Los campos nuevos aprobados → escalar al Architect (Opus) para registrarlos en ADR-0020 V2.
- **En cola (cimientos EPIC-0, tras la auditoría):** STORY-006 (`crash-recovery`, ojo solape con la recuperación de STORY-005), STORY-007 (`telemetry`), STORY-008 (`worker-isolation`), STORY-009 (CLI + binario raíz `app` + test `kill -9` real diferido). Transversal: los 6 spikes de gates (SPIKE-001–006) antes de cerrar EPIC-0 / arrancar EPIC-1.
