...
* **Tecnologías:** Única y exclusivamente **Rust**: Tokio (async), Rayon (paralelismo), **Polars** (DataFrames), **DuckDB** embebido (OLAP), **SQLite+SQLx** (OLTP WAL, migraciones embebidas), **Apache Arrow** (transporte binario), Serde (contratos Zero-Trust) y el motor de ejecución determinista (NautilusTrader/equivalente según veredicto Gate G1 del ROADMAP).
...
* Toda entidad de persistencia incluye los 25 campos del ADR-0020 V2 desde su primera migración.
