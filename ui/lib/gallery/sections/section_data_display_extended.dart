...
class GlowProgressCircular extends StatelessWidget {
  const GlowProgressCircular({
    super.key,
    this.value = 0.68,
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Stack(alignment: Alignment.center, children: [
        ...
        // Relleno del progreso con glow vía sombra del contenedor.
        CircularProgressIndicator(
          value: value,
          strokeWidth: 4,
          color: color,
        ...
        Text(
          '${(value * 100).round()}%',
          style: Gx.dataMono(fontSize: 11, color: color),
        ),
      ]),
    );
  }
...
class GlowTreeTable extends StatefulWidget {
  const GlowTreeTable({super.key});
  @override
  State<GlowTreeTable> createState() => _GlowTreeTableState();
}

class _GlowTreeTableState extends State<GlowTreeTable> {
  // Controla si el nodo padre está expandido.
  bool _expanded = true;
  ...
  Widget build(BuildContext context) {
    ...
    Widget row(List<Widget> cells, {Color? bg, bool child = false}) => Container(
          padding: EdgeInsets.symmetric(
              vertical: 6, horizontal: child ? 20 : 8),
          decoration: BoxDecoration(
      ...
      children: [
        ...
        GestureDetector(
          ...
          child: row([
            ...
            cell('—', num: false),
            cell('1.42', num: true, c: Gx.optimaCyan),
          ]),
        ),
        // Hijos del grupo.
        AnimatedSize(
          ...
          child: _expanded
              ? Column(children: [
                  row(
                    [cell('node-07'), cell('Tendencia', c: Gx.optimaCyan),
                      cell('1.84', num: true, c: Gx.optimaCyan)],
                    child: true,
                  ),
                  row(
                    [cell('node-12'), cell('Volátil', c: Gx.alertAmber),
...
class GlowCarousel extends StatefulWidget {
  const GlowCarousel({super.key});
  @override
  State<GlowCarousel> createState() => _GlowCarouselState();
}

class _GlowCarouselState extends State<GlowCarousel> {
  // Índice de la tarjeta actualmente visible.
  int _current = 0;
  final _controller = PageController();

  static const _cards = [
    ('node-07', 'Tendencia', Gx.optimaCyan),
    ('node-12', 'Volátil', Gx.alertAmber),
    ('node-19', 'Fallo', Gx.criticalCrimson),
  ];
